...
combine_mass_props_and_unc <- function(vl) {
  ...
  r$sigma_inertia = sqrt(Reduce(`+`, Map(
    f = function(v) {
      ...
      M_1 <- P - diag(c(p$x - 2 * p$y, p$y - 2 * p$x, p$z - 2 * p$x))
      M_2 <- t(P) - diag(c(p$x - 2 * p$z, p$y - 2 * p$z, p$z - 2 * p$y))

      Q <- outer(d, d)
      M_3 <- Q - sum(diag(Q)) * diag(3)

      v$sigma_inertia^2 + v$mass^2 * (M_1^2 + M_2^2) + (v$sigma_mass * M_3)^2
    },
    vl
  )))
...
add_radii_of_gyration_unc <- function(df) {
  Reduce(
    f = function(d, i) {
      ...
      sigma_k <- sqrt(sigma_I^2 / (mass * I) + (I * sigma_mass^2) / mass^3) / 2
