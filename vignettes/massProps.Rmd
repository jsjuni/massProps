...
$$
The SAWE reference gives equations for uncertainties of radii of gyration in recursive form, but as these radii are simply functions of moments of inertia and mass, we should be able to express their uncertainties in terms of uncertainties of moments of inertia and mass. Following standard uncertainty propagation technique:

$$
\begin{align}
{\sigma_\boldsymbol{k}}^2 & = \left( \frac{\partial \boldsymbol{k}}{\partial \boldsymbol{I}} \right)^2 {\sigma_\boldsymbol{I}}^2 + \left( \frac{\partial \boldsymbol{k}}{\partial m} \right)^2 {\sigma_m}^2 + \left( \frac{\partial \boldsymbol{k}}{\partial \boldsymbol{I}} \right) \left( \frac{\partial \boldsymbol{k}}{\partial m} \right) \sigma_{\boldsymbol{I}m} \\
  & = \left( \frac{1}{2 \sqrt{m \boldsymbol{I}}} \right)^2 {\sigma_\boldsymbol{I}}^2 + \left(\frac{- \sqrt{\boldsymbol{I}}}{2 m^{3/2}}\right)^2 {\sigma_m}^2 +  \left( \frac{1}{2 \sqrt{m \boldsymbol{I}}} \right) \left(\frac{- \sqrt{\boldsymbol{I}}}{2 m^{3/2}}\right) \sigma_{\boldsymbol{I}m} \\
  & = \frac{1}{4 m \boldsymbol{I}} {\sigma_\boldsymbol{I}}^2 + \frac{\boldsymbol{I}}{4 m^3} {\sigma_m}^2 - \frac{1}{4 m^2} \sigma_\boldsymbol{I} \sigma_m \rho_{\boldsymbol{I}m} \\
\end{align}
$$
where $\rho_{\boldsymbol{I}m}$ is the correlation coefficient of $\boldsymbol{I}$ and $m$. We can choose to assume that $\boldsymbol{I}$ and $m$ are independent, and therefore $\rho_{\boldsymbol{I}m} = 0$, but we can also reason as follows: all else being equal, as the mass of a composite increases, its moments of inertia increase as well. Therefore $\rho_{\boldsymbol{I}m} \ge 0$. Subject to that constraint, it is clear that the most conservative (i.e., the largest) estimate of ${\sigma_\boldsymbol{k}}^2$ is achieved for $\rho_{\boldsymbol{I}m} = 0$, in which case we have
...
```{r}
sigma_rog <- function(m, I, sigma_m, sigma_I) {
  sqrt(sigma_I^2 / (m * I) + (I * sigma_m^2) / m^3) / 2
}
sigma_ROG <- c(
