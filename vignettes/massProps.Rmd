...
Noting the repeated appearance of terms of the form $({x}_i - \bar{x})$, we form the outer product
...
$$
\begin{align}
{\sigma_k}^2 & = \left( \frac{\partial k}{\partial I} \right)^2 {\sigma_I}^2 + \left( \frac{\partial k}{\partial m} \right)^2 {\sigma_m}^2 \\
  & = \left( \frac{1}{2 \sqrt{m I}} \right)^2 {\sigma_I}^2 + \left(\frac{- \sqrt{I}}{2 m^{3/2}}\right)^2 {\sigma_m}^2 \\
  & = \frac{1}{4 m I} {\sigma_I}^2 + \frac{I}{4 m^3} {\sigma_m}^2 \\
\end{align}
$$
Let
...
$$
Recalling that arithmetic operations on vectors are element-wise, we can write the expression for radii of gyration uncertainty in vector form:
...
```{r}
sigma_mass <- sqrt(sum(sawe_input$σ_masslib^2))
```
...
4 mp          validation            1   0.954     0.936    0.019
3 mp       no validation            1   0.724     0.710    0.014
2 mp + unc    validation            1   1.621     1.596    0.025
1 mp + unc no validation            1   1.121     1.098    0.023
```
