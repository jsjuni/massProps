...
Let $\boldsymbol{s}_i$ be the matrix of inertia tensor summands in the standard formulas for a given subcomponent $i$ above. That is,
...
Note that the products of inertia formulas clearly employ the positive sign convention. Consequently, we negate those values in the inertia tensor. Finally,

$$
\boxed{
\boldsymbol{I} = \sum_{i=1}^{n} \{ \boldsymbol{I}_i
- 
w_i \left( \boldsymbol{Q}_i - \mathrm{tr}(\boldsymbol{Q}_i) \boldsymbol{I}_3 \right) \}
}
$$
...
If we construe (as `R` does) squaring and taking square roots or vectors element-wise, then
