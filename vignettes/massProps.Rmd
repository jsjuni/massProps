...
\end{align}
$$
where $\rho_{Im}$ is the correlation coefficient of $I$ and $m$. We can choose to assume that $I$ and $m$ are independent, and therefore $rho_{Im} = 0$, but we can also reason as follows: all else being equal, as the mass of a composite increases, its moments of inertia increase as well. Therefore $\rho_{Im} \ge 0$. Subject to that constraint, it is clear that the most conservative (i.e., the largest) estimate of ${\sigma_k}^2$ is achieved for $\rho_{Im} = 0$, in which case we have
