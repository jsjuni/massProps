...
\end{align}
$$
where $\sigma_{\boldsymbol{I}m}$ is the covariance between $\boldsymbol{I}$ and $m$ and $\rho_{\boldsymbol{I}m}$ is their correlation coefficient. We can choose to assume that $\boldsymbol{I}$ and $m$ are independent, and therefore $\rho_{\boldsymbol{I}m} = 0$, but we can also reason that, all else being equal, as the mass of a composite increases, its moments of inertia do not decrease. Therefore $\rho_{\boldsymbol{I}m} \ge 0$. Subject to that constraint and the fact that $\sigma_\boldsymbol{I} \sigma_m m^{-2} \gt 0$, it is clear that the most conservative (i.e., the largest) estimate of ${\sigma_\boldsymbol{k}}^2$ is achieved for $\rho_{\boldsymbol{I}m} = 0$, in which case we have
