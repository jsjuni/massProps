...
In this and the following code snippets, the variable `mpl` is a list of input mass property sets, the variable `mp` is a formal parameter of an anonymous function applied to each member of `mpl`, and `amp` is the resulting aggregate mass property set. The line above is an `R` functional programming idiom for "set the mass value of the result to the sum of the mass values of the inputs".
...
The SAWE reference gives equations for uncertainties of radii of gyration in recursive form, but as these radii are simply functions of moments of inertia and mass, we should be able to express their uncertainties in terms of uncertainties of moments of inertia and mass by applying standard uncertainty propagation theory.
...
$$
\begin{align}
X & = \sum_{i=1}^{n} (w_i + {\epsilon_w}_i) \\
Y & = \sum_{i=1}^{n} \left\{ {\Ixx}_i + {\epsilon_\Ixx}_i
  + (w_i + {\epsilon_w}_i) \left[ \left( {y}_i - \bar{y} \right)^2 + \left( {z}_i - \bar{z} \right)^2 \right]
  \right\}
 \\
\end{align}
$$
where $E[{\epsilon_w}_i {\epsilon_w}_j] = 0$ and $E[{\epsilon_\Ixx}_i {\epsilon_\Ixx}_j] = 0$ for all $i \neq j$ and $E[{\epsilon_w}_i {\epsilon_\Ixx}_j] = 0$ for all $i$ and $j$. It is clear from the linearity of $X$ and $Y$ in $w_i$ and $\Ixx_i$ that

$$
E[X] = E \left[\sum_{i=1}^{n} (w_i + {\epsilon_w}_i) \right] = w + \sum_{i=1}^{n} {\epsilon_w}_i = w
$$
and
$$
\begin{align}
E[Y] & = E \left[ \sum_{i=1}^{n} \left\{ {\Ixx}_i + {\epsilon_{\Ixx}}_i
...
$$
\begin{align}
\sigma_{\Ixx w} & = E \left[ \left( X - E[X] \right) \left( Y - E[Y] \right)\right] \\
                & = E \left[ \left( \sum_{i=1}^{n} {\epsilon_w}_i \right)
                    \left( \sum_{i=1}^{n} \left\{ {\epsilon_{\Ixx}}_i
                    + {\epsilon_w}_i \left[ \left( {y}_i - \bar{y} \right)^2 + \left( {z}_i - \bar{z} \right)^2 \right] \right\} \right) \right] \\
                & = E \left[ \sum_{i=1}^{n} {\epsilon_w}^2_i \left[ \left( {y}_i - \bar{y} \right)^2 + \left( {z}_i - \bar{z} \right)^2 \right] \right] \\
...
We can choose to assume that $\boldsymbol{I}$ and $w$ are independent, and therefore $\rho_{\boldsymbol{I}w} = 0$, but we can also note that, all else being equal, as the mass of a composite increases, its moments of inertia do not decrease. Therefore $\rho_{\boldsymbol{I}w} \ge 0$. Subject to that constraint and the fact that $\sigma_\boldsymbol{I} \sigma_w w^{-2} \gt 0$, it is clear that the most conservative (i.e., the largest) estimate of ${\sigma_\boldsymbol{k}}^2$ is achieved for $\rho_{\boldsymbol{I}w} = 0$, in which case we have
...
```{r}
sawe_result <- rollup_radii_of_gyration_unc(
  add_radii_of_gyration(
    rollup_mass_props_and_unc(sawe_tree, sawe_table)[3, ]
  )
)
sawe_result
```
...
```{r}
sigma_rog <- function(m, I, sigma_m, sigma_I, rho_Im) {
  sqrt(sigma_I^2 / (4 * m * I) + (I * sigma_m^2) / (4 * m^3) - (rho_Im * sigma_m * sigma_I) / (2 * m^2))
}
rho_Im <- 0
sigma_ROG <- c(
  sigma_kx = sigma_rog(sawe_result$mass, sawe_result$Ixx, sawe_result$sigma_mass, sawe_result$sigma_Ixx, rho_Im),
  sigma_ky = sigma_rog(sawe_result$mass, sawe_result$Iyy, sawe_result$sigma_mass, sawe_result$sigma_Iyy, rho_Im),
  sigma_kz = sigma_rog(sawe_result$mass, sawe_result$Izz, sawe_result$sigma_mass, sawe_result$sigma_Izz, rho_Im)
)
```
...
```{r}
result <- rollup_radii_of_gyration_unc(add_radii_of_gyration(rollup_mass_props_and_unc(sawe_tree, sawe_table)))
parts <- result[1:2, ]
aggr <- result[3, ]
sigma_rog_sawe_a <- function(k, kt, v1, c1, v2, c2, sigma_m, m, sigma_v1, sigma_v2, sigma_k) {
  ((k^2 - kt^2 + (v1 - c1)^2 + (v2 - c2)^2) * sigma_m)^2 +
    (2 * m * (v1 - c1) * sigma_v1)^2 +
    (2 * m * (v2 - c2) * sigma_v2)^2 +
    (2 * m * k * sigma_k)^2
}
sigma_rog_sawe_b <- function(k, m) 1 / (2 * k * m)
sigma_ROG_sawe <- c(
  sigma_kx = sigma_rog_sawe_b(aggr$kx, aggr$mass) * sqrt(sum(sigma_rog_sawe_a(parts$kx, aggr$kx, parts$Cy, aggr$Cy, parts$Cz, aggr$Cz,
                                                         parts$sigma_mass, parts$mass, parts$sigma_Cy, parts$sigma_Cz, parts$sigma_kx))),
  sigma_ky = sigma_rog_sawe_b(aggr$ky, aggr$mass) * sqrt(sum(sigma_rog_sawe_a(parts$ky, aggr$ky, parts$Cx, aggr$Cx, parts$Cz, aggr$Cz,
                                                         parts$sigma_mass, parts$mass, parts$sigma_Cx, parts$sigma_Cz, parts$sigma_ky))),
  sigma_kz = sigma_rog_sawe_b(aggr$kz, aggr$mass) * sqrt(sum(sigma_rog_sawe_a(parts$kz, aggr$kz, parts$Cx, aggr$Cx, parts$Cy, aggr$Cy,
                                                         parts$sigma_mass, parts$mass, parts$sigma_Cx, parts$sigma_Cy, parts$sigma_kz)))
)
```
